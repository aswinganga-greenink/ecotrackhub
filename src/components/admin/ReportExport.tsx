...
export function ReportExport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const dataResponse = await api.getMonthlyData();
  ...
  const generateCSV = () => {
    const data = monthlyData.filter(d => {
    ...
    data.forEach(entry => {
      let row = `Anjarakandi,${entry.month},${entry.year},${entry.electricityKwh},${entry.dieselLiters},${entry.petrolLiters},${entry.wasteKg},${entry.waterLiters},${entry.solarUnits},${entry.treesPlanted}`;
    ...
    a.href = url;
    a.download = `carbon_report_anjarakandi_${selectedYear}.csv`;
...
`;
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `carbon_report_anjarakandi_${selectedYear}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
        ...
        <CardContent className="space-y-4">
          ...
          <div className="pt-4 border-t">
            ...
            <div className="p-4 bg-muted rounded-lg text-sm">
              <p><strong>Region:</strong> Anjarakandi</p>
